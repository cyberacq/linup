...
show_help() {
    cat << EOF
Linux Updater with Multi-Distro Support - Installer v2.3
...
show_version() {
    echo "Linux Updater with Multi-Distro Support - Installer v2.3"
    exit 0
}
...
check_kernel_reboot_needed() {
    ...
    if [ -f /proc/version ]; then
        ...
        case "$PKG_MANAGER" in
            apt)
                installed_kernel=$(dpkg -l 2>/dev/null | grep linux-image | grep ^ii | tail -1 | awk '{print $2}' | sed 's/linux-image-//' || true)
                ;;
            dnf|yum)
                installed_kernel=$(rpm -q kernel 2>/dev/null | tail -1 | sed 's/kernel-//' || true)
                ;;
            pacman)
...
check_reboot_needed() {
    # Only check Debian/Ubuntu style reboot-required file
    if [ -f /var/run/reboot-required ]; then
        return 0
    fi
...
offer_reboot() {
    ...
    # Debian/Ubuntu provides a package list; other distros use kernel version comparison
    if [ -f /var/run/reboot-required.pkgs ]; then
        echo "Packages requiring reboot:"
        cat /var/run/reboot-required.pkgs | sed 's/^/  - /'
    else
        echo "  Reason: installed kernel differs from running kernel"
    fi
...
.TH linup 1 "May 2026" "Version 2.3" "User Commands"
.SH NAME
...
.IP 5.
Detect and offer system reboot if required
.SH EXAMPLES
.TP
Run the updater interactively:
.B sudo linup
...
main() {
    echo ""
    print_header "Linux Updater with Multi-Distro Support - Installer v2.3"
    echo ""
